...
class ProxyHandler {
    ...
    /**
     * Configure a cURL handle with a custom proxy
     * 
     * @param resource $ch cURL handle
     * @param array $proxy_config Custom proxy configuration
     * @return resource Configured cURL handle
     */
    public static function configureCurlWithCustomProxy($ch, $proxy_config) {
        if (!$proxy_config) {
            return $ch;
        }
        ...
        if (isset($proxy_config['type']) && $proxy_config['type'] === 'http') {
...
// Function to configure a curl session with proxy settings
function configureCurlWithProxy($ch, $proxy_config) {
    return ProxyHandler::configureCurlWithCustomProxy($ch, $proxy_config);
}
?>